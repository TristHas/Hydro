...
def __target_nc_creation(self):
    ...
    for nc_name in nc_names:
        ...
        target_date_times = time_var.data#nc4.num2date(time_var,units = time_unit)#,calendar = time_cal)
        target_name = self.output_dir + self.case_name #+ '_remapped_' + "givendate.nc"#target_date_times[0].strftime("%Y-%m-%d-%H-%M-%S")+'.nc'
        ...
        print("Checking if", self.save_csv)
        ...
        if self.save_csv:
            ...
            for i in np.arange(len(self.var_names_remapped)):
                ...
                target_dir = self.output_dir + self.var_names_remapped[i]
                os.makedirs(target_dir, exist_ok=True)
                target_name_csv =  target_dir + "/" + self.case_name.replace(".nc", '.csv')
                ...
                print(f"Dumped {target_name_csv}")
            ds.close()
            os.remove(target_name)
...
def grib_to_nc(infile, tmpdir="/tmp/ncl_convert"):
...
def process_grib(in_path, 
                 ...
                 varout=["temperature"],
                 outdir = '/media/tristan/Elements/Hydro/weather/MSM_basin_temp/',
                 remap_csv= './temporary/nc_remapping.csv',
                 target_shp="/media/tristan/Elements/Hydro/MERIT/MERIT_BASIN/dam_basins.shp"):  
...
#def _process_grib(x): './2008_01_01_00_00-15.nc'
...
def parallel_process_grib(indir, outdir, varin, varout, nproc=20):
    ...
    remaining = list(set([os.path.basename(x) for x in infiles]) \
                   - set([os.path.basename(x).replace(".csv", ".grib2") for x in os.listdir(outdir + varin[0])]))
